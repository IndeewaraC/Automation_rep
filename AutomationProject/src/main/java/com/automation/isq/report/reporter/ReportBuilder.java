...
package com.automation.isq.report.reporter;
...
import com.automation.isq.report.model.TestCase;
import com.automation.isq.report.model.TestExecution;
import com.automation.isq.report.model.TestStep;
...
public class ReportBuilder {
    ...
    public final void addNewTestCase(final String modulename,
        ...
        ThreadLocalClass.threadLocal.get().setCurrentTestCase(testCase);
        
        // This is to add Test Case to Test Suits.
        ThreadLocalClass.threadLocal.get().getCurrentTestSuit().getTestCases().add(testCase);
        
        /*
        for(TestSuite ts : testExecution.getTestSuites()){
            if(ts.getTestsuitename() != null && ts.getTestsuitename().equals(testSuitName)){
            	ts.getTestCases().add(testCase);
            	break;
            }
        }*/
    ...
    public final void addNewTestStep(final boolean isPassed,
        ...
        ThreadLocalClass.threadLocal.get().getCurrentTestCase().getTestSteps().add(testStep);
    ...
    public final void addNewTestStep(final boolean isPassed,
        ...
        if (!isPassed) {
        	ThreadLocalClass.threadLocal.get().getCurrentTestCase().setResult("Failed");
        	ThreadLocalClass.threadLocal.get().getCurrentTestSuit().setResult("Failed");
            ...
            for (TestCase tc : ts.getTestCases()) {
                ...
                if (!reportedTestCases.containsKey(tcId)) {
                	...
                	if(!result.equals(tc.getResult())){
                		if ("Failed".equals(tc.getResult())) { //previously Success
                			testExecution.setTotalfailedcount(1);
                			testExecution.setTotalerrorcount(1);
                			testExecution.setTotalsuccesscount(-1);
                		} else if ("Success".equals(tc.getResult())) { //Previously Failed
                			testExecution.setTotalsuccesscount(1);
                			testExecution.setTotalfailedcount(-1);
                			testExecution.setTotalerrorcount(-1);
                		}
                    	reportedTestCases.put(tcId, tc.getResult()); // Upgrade the result
                	}
                }
            }
